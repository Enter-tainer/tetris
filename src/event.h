...
// left arrow pressed -> while(is pressing) { if is first time && press_time > DAS move_left_step, wait ARR  }
...
// when lock_timer arrives, [lock_mino, disable lock_timer], [spawn_mino, reset down_timer]
